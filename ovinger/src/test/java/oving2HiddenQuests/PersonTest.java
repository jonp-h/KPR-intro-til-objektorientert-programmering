...
public class PersonTest {
	...
	void testSetEmail() {
		...
		testInvalidEmail("ola.nordmann@tiller", email, IllegalArgumentException.class);
		testInvalidEmail("ola.nordmann(at)tiller.no", email, IllegalArgumentException.class);
		testInvalidEmail("espen.askeladd@eventyr.no", email, IllegalArgumentException.class);
		Assertions.assertDoesNotThrow(() -> {
			person.setEmail("ola.nordmann@tiller.no");
		});
		assertEquals("ola.nordmann@tiller.no", person.getEmail());
	}
	...
	public void testExtraCountryTopLevelDomains() {
		...
		for (String cTLD : invalidCTLDs) {
			testInvalidEmail("john.doe@tiller." + cTLD, email, IllegalArgumentException.class);
		}
